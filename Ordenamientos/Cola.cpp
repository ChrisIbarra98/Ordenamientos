...
int Cola::busquedaBinaria(Alumno& elem)
{
 ...
 int Cola::particion(int inicio, int fin) {
        Alumno pivote = datos[inicio];
        int i = inicio - 1;
        int j = fin+1;
...
void Cola::quicksortS(int inicio, int fin)
 {
      if (inicio < fin) {
            int pivote = particion(inicio, fin);
            quicksortS(inicio, pivote);
            quicksortS(pivote + 1, fin);
        }
}
void Cola::quicksort()
 {
